...
class Main:
    ...
    def __init__(self):
        ...
        # load the scenes
        util.create_scene_manager()

        # pre loaded scenes
        util.scenes[MainMenu.name()] = MainMenu(self.screen)
        util.scenes.set_active_scene(MainMenu.name())

        self.run()

    def run(self):
        # Main Loop
        while util.scenes.is_scene_alive():
            self.screen.fill((0, 0, 0))
            active_scene = util.scenes.active_scene
            active_scene.update()
