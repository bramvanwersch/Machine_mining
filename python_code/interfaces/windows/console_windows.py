...
class ConsoleWindow(Window):
    ...
    _input_line: Union["ConsoleLine", None]
    _text_log_label: Union["TextLogLabel", None]
    _log: "TextLog"
    _console: consoles.Console
    ...
    def __init__(
        ...
        console_: consoles.Console,
        title="CONSOLE",
        static=False,
        color=(150, 150, 150),
        max_line_size=con.SCREEN_SIZE.width,
        **kwargs
    ):
        super().__init__(pos, size, sprite_group, static=static, title=title,
                         color=color, **kwargs)
        self._input_line = None
        self._text_log_label = None
        self._log = TextLog(max_line_size)
        self._console = console_
        self._max_line_size = max_line_size

        self.__init_widgets(size)
    ...
    def __set_log_line(
        ...
        if direction == 1:
            line = self._log.line_down()
        elif direction == -1:
            line = self._log.line_up()
        else:
            raise util.GameException(f"Invalid direction for retrieving Console line {direction}")
        self._input_line.active_line.set_line_text(line.text)

    def __init_widgets(
        self,
        size: util.Size
    ):
        self._input_line = ConsoleLine(size.width - 10)
        self.add_widget((5, size.height - self._input_line.rect.height - 5), self._input_line)
        self.add_border(self._input_line, color=(50, 50, 50))

        self._text_log_label = TextLogLabel(size - (0, self._input_line.rect.height + 10), self._log)
        self.add_widget((0, 0), self._text_log_label)
    ...
    def show_window(
        ...
        if is_showing is True:
            self.selected_widget = self._input_line
            self._input_line.set_selected(True)
    ...
    def update(
        ...
        *args: Any
        ...
        super().update()
        if self._input_line.process_line is not None:
            self.add_executed_command_message(self._input_line.process_line)
            results = self._console.process_line(self._input_line.process_line)
            ...
            self._input_line.process_line = None
    ...
    def __create_tab_information(self):
        ...
        is shared between all options"""
        # get all the text between the start and the cursor
        full_line = self._input_line.active_line.text
        until_cursor_text = full_line[:self._input_line.active_line.line_location]
        after_cursor_text = full_line[self._input_line.active_line.line_location:]
        commands = self._get_commands_from_line(until_cursor_text)
        possible_commands_dict = self._console.command_tree
        ...
        # if it ends on a perfect command simply add a space to the line
            ...
            self._input_line.active_line.append(" ")
            return
        else:
        ...
        if len(possible_commands) == 1:
            ...
            if len(last_command) == 0:
                self._input_line.active_line.append(possible_commands[0])
            else:
                self._input_line.active_line.set_line_text(until_cursor_text[:-len(last_command)]
                                                           + possible_commands[0] + after_cursor_text)
                self._input_line.active_line.set_line_location(len(until_cursor_text) - len(last_command) +
            ...
            if not len(last_command) == 0:
                self._input_line.active_line.set_line_text(until_cursor_text[:-len(last_command)] + letters +
                                                           after_cursor_text)
                self._input_line.active_line.set_line_location(len(until_cursor_text) - len(last_command) +
                                                               len(letters))
    ...
    def _get_commands_from_line(
        ...
        """Spit a line into words and return all the individual words, here for inheriting purposes"""
        commands = current_line_text.split(" ")
        return commands
    ...
    def add_error_message(
        ...
        message = "ERROR: "
        self._log.append_other(Line(text=message + text, color=(163, 28, 23), max_line_size=self._max_line_size))

    def add_result_message(
        self,
        text: str
    ):
        self._log.append_other(Line(text=text, color=(25, 118, 168), max_line_size=self._max_line_size))

    def add_tab_possibilities_message(
        self,
        text: str
    ):
        self._log.append_other(Line(text=text, color=(0, 0, 255), line_type="list", max_line_size=self._max_line_size))

    def add_executed_command_message(
        self,
        text: str
    ):
        """Print a command that was just executed in greenn"""
        self._log.append(Line(text=text, color=(64, 235, 52), max_line_size=self._max_line_size))
...
class TextLog:
    ...
    def line_up(self) -> "Line":
        ...
        if len(self.__user_log) == 0:
            return Line(max_line_size=self._max_line_size)
...
class Line:
    ...
    def __init__(
        ...
        max_line_size: int = con.SCREEN_SIZE.width
    ):
        self.text = text
        self.__color = color
    ...
    def __render_normal_line(self) -> List[str]:
        ...
        if self.__font.size(self.text)[0] > self._max_line_size:
            words = self.text.split(" ")
            lines = [""]
            line_length = 0
            for word in words:
                word_size = self.__font.size(word + " ")[0]
                if line_length + word_size < self._max_line_size:
                    lines[len(lines) - 1] += word + " "
                    line_length += word_size
                else:
                    line_length = self.__font.size(word)[0]
                    lines.append(word)
            return lines
        else:
            return [self.text]
    ...
    def __render_list_line(self) -> List[str]:
        ...
        be shown in such a way that the list elements allign under one another if there are multiple lines needed"""
        words = self.text.split(" ")
    ...
    def copy(self) -> "Line":
        ...
        return Line(text=self.text, color=self.__color, line_type=self.__line_type, max_line_size=self._max_line_size)


class MainConsoleWindow(ConsoleWindow):
    # for the main console window that is opened with ,

    def run_starting_script(self):
        self._console.process_line("scripts start")

    def _get_commands_from_line(
        self,
        current_line_text: str
    ) -> List[str]:
        """Spit a line into words and make sure that list multipliers are properly read"""
        word = ""
        commands = []
